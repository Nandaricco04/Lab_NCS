...
$err = '';
$id = (string)($_GET['id'] ?? $_GET['id_page'] ?? '');
...
try {
    $res = qparams('SELECT "id_page", "judul", "gambar_path" FROM "pages" WHERE "id_page"=$1', [$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['gambar_path']) && $_FILES['gambar_path']['error'] == UPLOAD_ERR_OK) {
        ...
        $nama_file_unik = 'home_' . time() . '_' . uniqid() . '.' . $ext;
    ...
    if ($Id_Page === '' || $Judul === '' || $gambar_path_baru === '') {
            ...
            <div class="flex-center">
                <div class="card-wrapper">
                    ...
                    <form class="form-user-box" method="post" enctype="multipart/form-data" autocomplete="off">
                        <div class="desc-form">
                            Ubah data home di bawah sesuai kebutuhan.
                        </div>
                        <?php if ($err): ?>
